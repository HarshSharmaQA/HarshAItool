...
export async function updateHomePage(idToken: string, formData: FormData) {
  ...
  const validatedFields = HomePageSchema.safeParse(rawData);
  ...
  try {
    ...
    await homePageRef.set(homePageData, { merge: true });
